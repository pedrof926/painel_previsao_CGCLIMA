...
def construir_mapa_sobreposicao(var_key: str, data_iso: str | None, camada_unidade: str, mostrar_previsao: bool, mostrar_unidades: bool) -> go.Figure:
    ...
    if mostrar_previsao:
        ...
        if not feats:
            ...
            for i, ft in enumerate(feats):
                ...
                # ✅ ALTERAÇÃO ÚNICA: usar properties.id (mais robusto no Plotly)
                ft2 = dict(ft)
                ...
                fig.add_trace(
                    go.Choroplethmapbox(
                        ...
                        locations=[ft2_props["id"]],    # ✅ ALTERAÇÃO ÚNICA
                        z=[1],
    ...
    fig.update_layout(
        ...
        ,  # <- mantém exatamente a estrutura que você tinha (só garantindo vírgula correta)
        paper_bgcolor="white",
        plot_bgcolor="white",
        showlegend=True,
        legend=dict(
